...
puts "Il y a #{array.size} journalistes."
puts ""
...
while i < array.size
	if array[i] =~ /\d/ #\d is the same as [0-9]
  		res = res + 1
	end
	i = i + 1
end

puts "Il y a #{res} handle qui contiennent un numéro." 
puts ""
...
array.each do |var|
	if var.include?("Aude") == true || var.include?("aude") == true
		res = res + 1
	end
end

puts "Il y a #{res} handle qui contiennent les 4 lettres du prénom Aude." 
puts ""
...
array.each do |var|
	if var[1].upcase == var[1]
		res << ""
	end
end

puts "Il y a #{res.size} handle qui commencent par une majuscule." 
puts ""

#5 : Combien contiennent une majuscule ?
res = array.grep(/[A-Z]/).count # .grep isole les val entre A et Z

puts "Il y a #{res} majuscules." 
puts ""

#6 : Combien de _ dans tous les pseudos confondus ?
string = array.join #transforme array en string

res = string.scan(/_/).count #scan et compte les _

puts "Il y a #{res} _ dans tous les pseudos confondus."
puts ""
